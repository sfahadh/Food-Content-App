...
export default class FoodContent extends Component {
  ...
  handleSearchInput(e) {
    let filteredFood = this.props.nutrientContent.filter(searchedFood => {
      return (
        searchedFood.name.toLowerCase().search(e.target.value.toLowerCase()) !==
        -1
      );
    });
    this.setState({
  ...
  render() {
    ...
    let table = (
      <div className="nutrition-content">
        ...
        <div className="box">
          {(() => {
            if (selectFood !== undefined) return <p>{selectFood.weight}</p>;
          })()}
        </div>
        <div className="box">
          {(() => {
        ...
        <div className="box">
          {(() => {
            if (selectFood.nutrients[1].value !== undefined) {
              return parseInt(selectFood.nutrients[1].value !== typeof 1) ||
                selectFood.nutrients[1].value === "--" ? (
                <p>0</p>
              ) : (
                <p>
                  {parseInt(Math.round(selectFood.nutrients[1].value) * 4) / 4}
                </p>
              );
            } else {
              return <p />;
        ...
        <div className="box">
          {(() => {
            if (selectFood.nutrients[3].value !== undefined) {
              return parseInt(selectFood.nutrients[3].value !== typeof 1) ||
                selectFood.nutrients[3].value === "--" ? (
                <p>0</p>
              ) : (
                <p>
                  {parseInt(Math.round(selectFood.nutrients[3].value) * 4) / 4}
                </p>
              );
            } else {
              return <p />;
        ...
        <div className="box bottom">
          {(() => {
            if (selectFood.nutrients[2].value !== undefined) {
              return parseInt(selectFood.nutrients[2].value !== typeof 1) ||
                selectFood.nutrients[2].value === "--" ? (
                <p>0</p>
              ) : (
                <p>
                  {parseInt(Math.round(selectFood.nutrients[2].value) * 9) / 9}
                </p>
              );
            } else {
              return <p />;
    ...
    return (
      <div id="foodTracker-background">
        ...
        {foodNames.map(foodName => {
          return (
            <li id="food-name" key={foodName.ndbno}>
              <Link
                style={{ color: "orange" }}
